...
const tools = [
  ...
  },
  {
    ...
    status: 'Built',
    statusClass: 'small',
    desc: 'MCP server for utility operations. Wraps existing bash scripts into typed, validated mcporter calls. Not a data store — just functions.',
    details: [
      { label: 'Type', value: 'MCP Server (Node.js, no database)' },
      { label: 'Tools', value: '4 tools: camera_snapshot, weather_fetch, panel_update, avatar_set' },
      { label: 'Repo', value: 'Local: ~/york/york-tools/' },
      { label: 'Access', value: 'mcporter call york-tools.<tool>(...)' },
    ],
    usedBy: 'York (camera, panel, avatar, weather), Dagr (weather), Hild (camera for chore state).',
  },
  {
    icon: '✅',
    name: 'google-tasks',
  ...
  {
    icon: '🎨',
    name: 'image-gen (york-tools)',
    color: 'green',
    status: 'Built',
    statusClass: 'large',
    desc: 'Multi-model image generation via york-tools MCP. Currently Gemini family (Pro, Flash 3.1, Flash 2.5). Extensible for SD, Flux, Grok, etc. Per-model parameter validation — models declare what they support.',
    details: [
      { label: 'Tools', value: 'image_generate (generate images), image_models (list models + capabilities)' },
      { label: 'Models', value: 'gemini-3-pro-image-preview (best), gemini-3.1-flash-image-preview (fast), gemini-2.5-flash-image (fastest)' },
      { label: 'Params', value: 'Core: prompt, model. Model-dependent: aspect_ratio, resolution, negative_prompt, cfg_scale, steps, seed, reference_image' },
      { label: 'Output', value: '~/.openclaw/gen/gen-<timestamp>.png' },
      { label: 'Skills', value: 'Global: shared-skills/image-gen/. D&D-specific: workspace-caedmon/skills/image-gen/' },
    ],
    usedBy: 'Caedmon (D&D art), any agent via global skill. Future: SD/Flux models added to provider registry.',
  },
  {
    icon: '📹',
    name: 'camera-monitor',
