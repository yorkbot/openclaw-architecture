...
const tools = [
  ...
  },
  {
    ...
    details: [
      ...
      { label: 'Functions', value: 'camera_snapshot, weather_fetch, panel_update, avatar_set' },
      { label: 'Why MCP', value: 'One interface. Agents call mcporter, not bash scripts.' },
    ],
    usedBy: 'All agents as needed.',
  },
  {
    icon: '✅',
    name: 'google-tasks',
  ...
  },
  {
    ...
    name: 'image-gen',
    color: 'yellow',
    status: 'Exists as script, needs MCP',
    statusClass: 'medium',
    desc: 'Image generation via Gemini. Currently a bash script — needs to be wrapped as an MCP tool for consistent mcporter access.',
    details: [
      { label: 'Type', value: 'MCP Server (wrapping Gemini API)' },
      { label: 'Model', value: 'Gemini Nano Banana Pro (gemini-2.0-flash-exp)' },
      { label: 'Input', value: 'Text prompt describing the image' },
      { label: 'Output', value: 'Generated image file path' },
    ],
    usedBy: 'Caedmon (D&D art — characters, locations, items, scenes).',
  },
  {
    icon: '📹',
    name: 'camera-monitor',
  ...
  },
  {
    ...
    status: 'Built (legacy)',
    statusClass: 'small',
    desc: 'Google Sheets MCP server. Still in mcporter for legacy access. Health data has migrated to york-data. Home projects spreadsheet still uses Sheets.',
    details: [
      { label: 'Type', value: 'MCP Server' },
      { label: 'Tools', value: '17 tools' },
      { label: 'Access', value: 'mcporter call google-sheets.<tool>(...)' },
    ],
    usedBy: 'Hild (home projects spreadsheet). Legacy health data access being phased out.',
  },
  {
    icon: '🎲',
    name: 'dice-roller',
