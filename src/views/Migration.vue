...
const phases = [
  {
    ...
    steps: [
      ...
      { step: 'Flash SD card', details: 'Use <code>Raspberry Pi Imager</code> on the VM or another machine. In settings (gear icon): enable SSH, set hostname (e.g., <code>yorks-pi</code>), configure WiFi if needed. Don\'t set the username here — we\'ll create it manually.', verify: 'Pi boots, you can SSH in (default user varies by image — may be ubuntu/ubuntu)' },
      { step: 'Create york user', details: 'If the default user isn\'t <code>york</code>:<br><code>sudo adduser york</code><br><code>sudo usermod -aG sudo york</code><br>Log out, SSH back in as york. Optionally delete the default user after: <code>sudo deluser --remove-home ubuntu</code>', verify: 'whoami returns york, sudo works' },
      { step: 'SSH keys', details: 'From your machine: <code>ssh-copy-id york@yorks-pi</code> for passwordless login.<br><br>Copy yorkbot\'s SSH key for git operations:<br><code>scp -r ~/.ssh/id_* york@yorks-pi:~/.ssh/</code><br><code>scp ~/.ssh/config york@yorks-pi:~/.ssh/</code><br><code>ssh yorks-pi "chmod 700 ~/.ssh && chmod 600 ~/.ssh/id_*"</code>', verify: 'ssh york@yorks-pi works without password. ssh -T git@github.com shows "Hi yorkbot!"' },
      { step: 'First boot + updates', details: '<code>sudo apt update && sudo apt upgrade -y</code><br>Then: <code>sudo apt install -y git curl build-essential sqlite3 python3 python3-pip zsh</code><br>Optional: <code>chsh -s $(which zsh)</code> for zsh (VM uses zsh)', verify: 'git --version, sqlite3 --version, python3 --version all return' },
  ...
  },
  {
    ...
    steps: [
      ...
      { step: 'Install gh CLI', details: '<code>sudo apt install -y gh</code> (or via GitHub\'s official APT repo if the Ubuntu version is old).<br><code>gh auth login</code> — authenticate as yorkbot (use SSH protocol).', verify: 'gh auth status shows logged in as yorkbot' },
      { step: 'Git config', details: '<code>git config --global user.name "yorkbot"</code><br><code>git config --global user.email "yorkbot@users.noreply.github.com"</code>', verify: 'git config --global --list shows correct name and email' },
    ],
  },
  {
    number: 3,
  ...
  },
  {
    ...
    steps: [
      { step: 'Vault watcher', details: 'Install npm dependency first:<br><code>cd ~/.openclaw/workspace-wiglaf/tools && npm install</code><br>Copy service file:<br><code>cp ~/.config/systemd/user/vault-watcher.service</code> (already in ~/.openclaw dir from rsync)<br>Or create from template — see inventory below.<br><code>systemctl --user daemon-reload</code><br><code>systemctl --user enable --now vault-watcher</code><br><br><strong>Important:</strong> Uses chokidar (not raw fs.watch/inotify) because Syncthing atomic renames (write temp → rename into place) silently break inotify watches. Must use directory paths, not globs (chokidar globs can fail silently on some filesystems). The awaitWriteFinish option handles Syncthing temp→rename writes.', verify: 'systemctl --user status vault-watcher shows active. Simulate Syncthing atomic write: <code>cp file .tmp && echo test >> .tmp && mv .tmp file</code> — check logs show dispatch within debounce window (60s for Daily/).' },
