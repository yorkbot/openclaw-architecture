...
const phases = [
  ...
  },
  {
    ...
    changes: [
      'Update orchestrator-dispatch skill with model routing rules per complexity tier',
      'Consumption logging sub-agents spawn with a Small tier model',
      'Research sub-agents spawn with model based on task complexity (York decides at spawn time)',
      'Builder tasks spawn with XL tier model',
      'Set up API keys for chosen providers (decide which models fill each tier)',
    ],
    risks: [
      'Smaller models may handle some consumption edge cases worse (odd food descriptions, unit conversions)',
      'Need to validate that Small tier models follow skill files reliably',
    ],
    validation: 'Run a week with right-sized models. Compare sub-agent success rate and output quality. Check for any regressions in data accuracy.',
  },
  {
    num: 3,
    ...
    changes: [
      ...
      'Write DnD SOUL.md (lore expert persona, deep knowledge of the interconnected campaign)',
    ...
    risks: [
      'DnD conversations that reference personal context (schedule, mood) lose that context — York can inject it when spawning',
  ...
  },
  {
    ...
    changes: [
      ...
      'York spawns Health agent (Small tier) for food logging, calorie checks, gate data',
      'Weekly review agent (Large tier) reads from same spreadsheet',
    ],
    risks: [
